...
// Hardcoded fallbacks for the most common Solana SPL tokens so our wallet
// import still labels them correctly when CoinGecko is rate-limiting us.
const SOLANA_MINT_FALLBACKS: Record<string, { coingecko_id: string; symbol: string; name: string }> = {
  EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v: { coingecko_id: 'usd-coin', symbol: 'USDC', name: 'USDC' },
  Es9vMFrzaCERmJfrF4H2FYD4KCoNkY11McCe8BenwNYB: { coingecko_id: 'tether', symbol: 'USDT', name: 'Tether' },
  So11111111111111111111111111111111111111112:   { coingecko_id: 'wrapped-solana', symbol: 'wSOL', name: 'Wrapped Solana' },
  DezXAZ8z7PnrnRJjz3wXBoRgixCa6xjnB7YaB1pPB263: { coingecko_id: 'bonk', symbol: 'BONK', name: 'Bonk' },
  JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbKedZNsDvCN:  { coingecko_id: 'jupiter-exchange-solana', symbol: 'JUP', name: 'Jupiter' },
  '4k3Dyjzvzp8eMZWUXbBCjEvwSkkk59S5iCNLY3QrkX6R': { coingecko_id: 'raydium', symbol: 'RAY', name: 'Raydium' },
  orcaEKTdK7LKz57vaAYr9QeNsVEPfiu6QeMU1kektZE:  { coingecko_id: 'orca', symbol: 'ORCA', name: 'Orca' },
  mSoLzYCxHdYgdzU16g5QSh3i5K3z3KZK7ytfqcJm7So:  { coingecko_id: 'msol', symbol: 'mSOL', name: 'Marinade Staked SOL' },
  HZ1JovNiVvGrGs7igeECtKiGHCFwHd9YTFGMCdFMkiuQ: { coingecko_id: 'pyth-network', symbol: 'PYTH', name: 'Pyth Network' },
};

// Mirror of app/coingecko.py::match_tokens_by_contract. Looks up CoinGecko
// metadata for a list of contract addresses on a given platform, with
// concurrency cap + 429 backoff. Known Solana mints have hardcoded fallbacks
// so the common case still works when CG is rate-limiting.
export async function matchTokensByContract(
  ...
  async function matchOne(address: string): Promise<void> {
    // Fallback for well-known Solana mints — use the hardcoded entry as the
    // base and enrich with a live price via fetchPrices if CG will return one.
    const fallback =
      platform === 'solana' ? SOLANA_MINT_FALLBACKS[address] : undefined;
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        clearTimeout(timer);
        if (resp.status === 404) return; // not listed
        if (resp.status === 429) {
          if (attempt < 2) {
        ...
        // Key by the address as-provided — Solana mints are base58 and
        // case-sensitive, while EVM addresses have already been lowercased
        // upstream (etherscan.ts) so both paths produce stable keys.
        out[address] = {
    ...
    // Live lookup failed — fall back to hardcoded mint if we have one, and
    // try to pull a price via the /coins/markets batched endpoint.
    if (fallback) {
      const priceData = await fetchPrices([fallback.coingecko_id]).catch(
        () => ({} as Record<string, PriceRecord>),
      );
      const p = priceData[fallback.coingecko_id];
      out[address] = {
        coingecko_id: fallback.coingecko_id,
        name: fallback.name,
        symbol: fallback.symbol,
        current_price_usd: p?.current_price_usd ?? null,
...
// Cache of resolved coin → platform → contract maps to avoid repeat hits.
const platformsCache = new Map<string, Record<string, string>>();

// Hardcoded fallbacks for the top tokens on common chains — used when
// CoinGecko's /coins/{id} endpoint is rate-limiting. Keyed by coingecko_id,
// value is { platform: contract_address }.
const PLATFORM_FALLBACKS: Record<string, Record<string, string>> = {
  ...
  ethereum: {
    // WETH9 on each EVM chain (wrapped ETH — the ERC-20 form of ETH)
...
// Resolve the on-chain contract/mint address for a CoinGecko coin on a
// given platform (e.g. 'ethereum', 'binance-smart-chain', 'solana'). Used
// to backfill coins imported before we started storing contract_address.
export async function resolveContractAddress(
  coingeckoId: string,
  platform: string,
): Promise<string | null> {
  // Try live CG first so we always get the freshest data when it's available.
  let platforms = platformsCache.get(coingeckoId);
  if (!platforms) {
    try {
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), 10_000);
      const resp = await fetch(
        `${BASE}/coins/${coingeckoId}?localization=false&tickers=false&market_data=false&community_data=false&developer_data=false&sparkline=false`,
        { signal: controller.signal },
      );
      clearTimeout(timer);
      if (resp.ok) {
        const data = (await resp.json()) as { platforms?: Record<string, string> };
        platforms = data.platforms ?? {};
        platformsCache.set(coingeckoId, platforms);
      }
    } catch {
      // fall through to fallback map
    }
  }
  const liveAddress = platforms?.[platform];
  if (liveAddress && liveAddress.length > 0) return liveAddress;

  // CG unavailable or no entry for this platform — try our hardcoded map.
  const fallback = PLATFORM_FALLBACKS[coingeckoId]?.[platform];
